...
# (Opcional) CSS extra — mantenha vazio por ora
...
def color_saldo(val):
    """Styler: vermelho se negativo, azul se positivo."""
    if pd.isna(val):
        return ""
...
def load_tbl1() -> pd.DataFrame:
    ...
    # Tipagem e colunas (mantém seu contrato atual)
...
def load_tbl2() -> pd.DataFrame:
    p = Path("data-processed/tbl_pagina1_tabela2_2026.csv")
    if not p.exists():
...
def load_intervencao() -> pd.DataFrame:
    p = Path("data-processed/tbl_pagina2_intervencao_2026.csv")
    if not p.exists():
        st.error(
...
with tab1:
    ...
    with st.expander("🔎 Filtros Avançados", expanded=True):
        ...
        with flt_cols[1]:
            sel_uo_cod = st.multiselect(
                "UO (Cód)", options=sorted(df_ano["UO cod"].unique()))
        with flt_cols[2]:
            sel_uo_sigla = st.multiselect("UO (Sigla)", options=sorted(
                df_ano["UO sigla"].dropna().unique()))
        with flt_cols[3]:
            sel_fonte = st.multiselect(
                "Fonte", options=sorted(df_ano["Fonte"].unique()))
        with flt_cols[4]:
            sel_ipu = st.multiselect(
                "IPU", options=sorted(df_ano["IPU"].unique()))
    ...
    st.dataframe(styler, column_config=column_cfg,
...
with tab2:
    ...
    uo = c1.multiselect("UO", sorted(df2["uo_cod"].unique()))
    acao = c2.multiselect("Ação", sorted(df2["acao_cod"].unique()))
    fonte = c3.multiselect("Fonte", sorted(df2["fonte_cod"].unique()))
    ipu = c4.multiselect("IPU", sorted(df2["ipu_cod"].unique()))
    m = (
        (df2["uo_cod"].isin(uo) if uo else True) &
        (df2["acao_cod"].isin(acao) if acao else True) &
        (df2["fonte_cod"].isin(fonte) if fonte else True) &
        (df2["ipu_cod"].isin(ipu) if ipu else True)
    )
    df2_f = df2[m].copy()
    ...
    st.download_button("⬇️ Baixar (.csv)", data=to_csv_bytes(df2_f),
                       file_name="pagina1_tabela2_2026.csv", mime="text/csv")
...
with tab3:
    ...
    c1, c2, c3, c4 = st.columns(4)
    uo = c1.multiselect("UO", sorted(df3["uo_cod"].unique()))
    acao = c2.multiselect("Ação", sorted(df3["acao_cod"].unique()))
    fonte = c3.multiselect("Fonte", sorted(df3["fonte_cod"].unique()))
    ipu = c4.multiselect("IPU", sorted(df3["ipu_cod"].unique()))
    m = (
        (df3["uo_cod"].isin(uo) if uo else True) &
        (df3["acao_cod"].isin(acao) if acao else True) &
        (df3["fonte_cod"].isin(fonte) if fonte else True) &
        (df3["ipu_cod"].isin(ipu) if ipu else True)
    )
    dff = df3[m].copy()
    ...
    st.download_button("⬇️ Baixar (.csv)", data=to_csv_bytes(dff),
                       file_name="pagina2_intervencao_2026.csv", mime="text/csv")
